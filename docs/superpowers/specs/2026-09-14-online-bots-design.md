...
Bots are always driven; abandoned seats keep the stricter existing rule, where one away player among present ones is just someone the table is still waiting for. The two rules converge when both kinds of seat are waiting: the bot moves on one alarm, the abandoned seat on the next.

Checking bots first is load-bearing even when only one seat is waiting, which is every case the shipping adapters actually produce today — Estate's `waitingOn` returns the trade's recipient *instead of* the current player, not both, and every other adapter returns a single seat too. When that one waiting seat is a bot, the abandoned-seat rule cannot help: `every(abandoned)` is false, because a bot is not abandoned. Without the bot clause the room would return `null` and the table would sit there forever. The clause is written to scan the whole waiting list rather than inspect `waiting[0]` so that an adapter which does block on several seats at once — a trade offered to a bot while its proposer still holds the turn — is handled the day one exists. `playStandIn` becomes `playAuto` and keeps the existing `standInStalls` counter and `MAX_STAND_IN_STALLS` backstop, which already protect against an adapter that never settles.

  ...
  return (
    <div className="hh-setup">
        ...
        {PLAYER_IDS.slice(0, count).map((id, index) => (
          ...
          >
            ...
            <select
              ...
              value={config.controls?.[index] || 'human'}
              onChange={(e) => {
                const controls = PLAYER_IDS.map((_, i) => config.controls?.[i] || 'human')
      ...
      <p className="hh-local-note">
        <Users size={13} />{' '}
        {config.controls?.slice(0, count).some((c) => c !== 'human')
          ? 'A little clever company. All on this device.'
          : 'Pass the device. Share the moment.'}
      </p>
    </div>
  )
}

...
function finishRoll(state: GameState, value: number) {
  ...
  for (let i = 0; i < 10 && next.phase !== 'ready' && next.phase !== 'won'; i++) {
    next = gameReducer(next, { type: next.phase === 'moving' ? 'STEP_DONE' : next.phase === 'transporting' ? 'TRANSPORT_DONE' : 'NEXT_TURN' })
  }
  return next
}
...
describe('Wildrise rules', () => {
  ...
  it('guards repeated rolls and phase-inappropriate actions', () => {
    ...
    expect(gameReducer(game, { type: 'STEP_DONE' })).toBe(game)
  ...
  it('advances one numbered square per completed step', () => {
    let game = gameReducer(gameReducer(createGame(), { type: 'ROLL', value: 3 }), { type: 'DICE_SETTLED' })
    expect(game.players[0].position).toBe(0)
    for (const position of [1, 2, 3]) {
      game = gameReducer(game, { type: 'STEP_DONE' })
      expect(game.players[0].position).toBe(position)
    }
    expect(game.phase).toBe('settling')
    expect(gameReducer(game, { type: 'NEXT_TURN' }).currentPlayer).toBe(1)
  })
  ...
  it('honors non-exact finish, optional bonus sixes and configurable routes', () => {
    const game = createGame({ rules: { exactFinish: false, extraTurnOnSix: true, snakeCount: 0, ladderCount: 0 } })
  ...
  it.each([2, 3, 4])('completes a %i-player game with fair seeded rolls and no invalid positions', playerCount => {
    let seed = 713

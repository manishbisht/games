...
async function startBotGame(page: Page, players = 2, name = 'Alex') {
  ...
  await expect(page).toHaveURL(/\/estate\/room\/[A-Z2-9]{6}$/, { timeout: 30000 })
...
test('play vs bot deals a real table, and the server takes the bots’ turns', async ({ page }) => {
  ...
  await expect(page.getByText('Alex').first()).toBeVisible({ timeout: 30000 })
  ...
  await expect(page.getByRole('button', { name: /Roll dice/ })).toBeEnabled({ timeout: 30000 })
  await page.getByRole('button', { name: /Roll dice/ }).click()
  // The dice are the server's, and the table is told what they were.
  await expect(page.locator('.activity-event').first()).toBeVisible({ timeout: 30000 })
...
test('a quick game is dealt quick, not classic', async ({ page }) => {
  ...
  await expect(page.getByText('$1,000').first()).toBeVisible({ timeout: 30000 })
